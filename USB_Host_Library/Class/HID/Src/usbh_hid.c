...
{	
  ...
  else
  {
    ...
    phost->pActiveClass->pData = (HID_HandleTypeDef *)USBH_malloc (sizeof(HID_HandleTypeDef));  //分配地址空间到 pdata 分配一个HID类
    HID_Handle =  (HID_HandleTypeDef *) phost->pActiveClass->pData; 
    HID_Handle->state = HID_ERROR;                                                              //默认状态 HID_ERROR
    
    /*Decode Bootclass Protocol: Mouse or Keyboard 解码boot类协议 鼠标or键盘 */
...
{   
  ...
  case HID_REQ_SET_IDLE:
    ...
    classReqStatus = USBH_HID_SetIdle (phost, 100, 0);//100
